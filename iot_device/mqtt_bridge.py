...
API_URL = os.environ.get("API_URL", "http://localhost:8000/lecturas/") #Alternativa: os.getenv("API_URL", "http://localhost:8000/lecturas/")
LOGIN_URL = API_URL.replace("/lecturas/", "/token")

form_data = {"username": "admin", "password": "12345"}  
TOKEN = None

# MECANISMO DE ESPERA Y REINTENTO (Evita que muera si el backend tarda en iniciar)
print("⏳ Conectando al backend para obtener el token de acceso...")
for i in range(10):  # Intenta 10 veces (espera total de hasta 30 segundos)
    try:
        token_recibido = requests.post(LOGIN_URL, data=form_data, timeout=5)
        if token_recibido.status_code == 200:
            TOKEN = token_recibido.json().get("access_token")
            print("🔑 [ÉXITO] Token de autenticación obtenido correctamente.")
            break
    except requests.exceptions.ConnectionError:
        print(f"⚠️ El backend aún no responde. Reintentando en 3 segundos... ({i+1}/10)")
        time.sleep(3)

if not TOKEN:
    print("❌ [CRÍTICO] No se pudo obtener el token de acceso. Abortando.")
    sys.exit(1)
